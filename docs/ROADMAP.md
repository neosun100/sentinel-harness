...
- [~] `[EXTERNAL]` cross-session managed Memory — **write + hard multi-tenant isolation proven live**; **semantic recall pending async extraction.** Live: CreateMemory (SEMANTIC + SUMMARIZATION strategies) → ACTIVE; CreateEvent wrote a disposition under `actorId=tenant-1` across two sessions; `tenant-1` sees its events, `tenant-2` sees **0** (isolation holds). The session-B-recalls-session-A step depends on SEMANTIC extraction, which is **asynchronous and service-scheduled** (`CreateEvent extractionMode` only offers `SKIP`; 0 jobs surfaced in a ~20-min window) — not a permission/gate issue, matches the documented minutes-scale extraction limitation. Honestly left unproven-in-window, not faked. — `evidence/live_memory_isolation_result.json`
...
- [x] `[EXTERNAL]` **managed Evaluate LLM-as-a-judge — proven (control plane).** A live SESSION-level, numerical, safety-aware CVE-triage `Evaluator` is ACTIVE (version-pinned Haiku judge; groundedness+safety as first-class dims, mirroring `loop_safety.apply_safety_veto`). — `evidence/live_managed_evaluator_result.json`. *Remaining:* `CreateOnlineEvaluationConfig` wired to a live OTEL trace dataSource so the judge scores emitted spans continuously (needs the running span pipeline); the flagship parallel 3-specialist A2A scan over real Runtime remains **blocked** by the account-side `CreateAgentRuntime` service gate (validated once on a prior date — `evidence/live_a2a_runtime_result.json`).
...
- [ ] `[EXTERNAL]` **fully autonomous closed loop — BLOCKED by a service-side data-plane gate.** The loop's control-plane spine is proven live (CreateHarness→READY, CreateHarnessEndpoint promote), but the autonomous *score→iterate* step needs a live `InvokeHarness` agent turn, and `InvokeHarness` returns `AccessDeniedException` at call time on this account **even though `iam:SimulatePrincipalPolicy` reports it `allowed`** (principal holds AdministratorAccess). Same service-side account opt-in gate class as `CreateAgentRuntime` — not self-resolvable, not faked. Root-cause captured in `evidence/live_dataplane_gate_diagnosis.json`. Reopens the moment the account's invoke data plane is allowlisted.
