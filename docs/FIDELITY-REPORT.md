...
- **Scale.** 740 offline tests pass (+4 skipped when optional deps absent) across 38 test files, with 14 evidence JSON artifacts, 9 scenarios, 9 tools, an `iac-cdk` project (8 stacks synth-green) and an `iac-terraform` mirror (`validate`-clean).
...
- **Sample detonation is a SIMULATED skeleton.** `longrunning/detonation/` is import-safe and returns SIMULATED no-ops. There is **no real malware, no real microVM, and no real network**. The destroy-after-use, sandbox-gate, and HITL wrappers around it are real; the detonation itself is not.
- **A2A specialist container is not built.** The LiteLLM / A2A specialist Runtime (`specialists/cve-intel/`) is an import-safe skeleton — the agent card is tested, but the container dependencies and Docker image are not built, so it does not serve over A2A.
...
- **Registry CFN type is not yet registered.** The Gateway and Memory native CFN types are registered, but the Registry type is not yet in CloudFormation. The Registry stack **synthesizes** but would fail on a real `deploy` until the type is registered.
...
- **`evidence/`** — 14 committed JSON artifacts from live and offline runs; `evidence/README.md` records what each captures and its region/run context.
